	...
	while true
		...
		elseif input == "help"
			...
			elseif function_number == 5
				println("write_angle_in_multiple_slices(atom_1, atom_2, iX, iY, iZ, fX, fY, fZ, Nz) has been selected.\n")
				println("Arguments are required.\n")
				println("Please enter them atom_1   atom_2   iX   iY   iZ   fX   fY   fZ   Nz.\n")
				println("As a reference, the dimensions of the NVT simulation box are")
				print("0.0   0.0   0.0")
            ...
            print(output)
				...
				atom_1 = args[1]
				atom_2 = args[2]
				iX, iY, iZ, fX, fY, fZ, Nz = parse.(Float64, args[3:end])

				stop_channel = Channel(1)
				progress_task = @async show_progress(stop_channel)
				write_angle_in_multiple_slices(atom_1, atom_2, iX, iY, iZ, fX, fY, fZ, Nz)
				put!(stop_channel, true)
				@async progress_task
