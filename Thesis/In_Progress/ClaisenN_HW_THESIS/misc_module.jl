...
function get_atomic_charges()
	return Dict(
		...
		"OW" => -0.7752308291*0.8424,
		"HW" => 0.7752308291*0.4212,
		"H00" => 0.9216292875*0.1360,
		"C01" => -0.9216292875*0.2273,
		"C02" => 0.9216292875*0.1473,
		"C03" => -0.9216292875*0.2259,
		"N04" => -0.9216292875*0.8995,
		"H05" => 0.9216292875*0.1067,
		"H06" => 0.9216292875*0.1067,
		"C07" => 0.9216292875*0.2676,
		"H08" => 0.9216292875*0.4502,
		"C09" => -0.9216292875*0.2279,
		"C0A" => -0.9216292875*0.2158,
		"H0B" => 0.9216292875*0.1415,
		"C0C" => -0.9216292875*0.0972,
		"H0D" => 0.9216292875*0.1490,
		"C0E" => -0.9216292875*0.0995,
		"H0F" => 0.9216292875*0.1426,
		"C0G" => -0.9216292875*0.2015,
		"H0H" => 0.9216292875*0.1417,
		"H0I" => 0.9216292875*0.1475,
		"H0J" => 0.9216292875*0.1289,
		"H0K" => 0.9216292875*0.1289,
							)
end
...
function electric_field_at_point(molecule_name, atom_1, atom_2, iX, iY, iZ, fX, fY, fZ)
   ...
   Lz = fZ - iZ
	...
	for time in sort(collect(keys(results)))
		...
		distance_vector = atom_2_coords .- atom_1_coords
...
function electric_field_at_point_all(atom_1, atom_2, iX, iY, iZ, fX, fY, fZ)
	k = 1.43996454 # Coulomb's constant in (eV nm^2 {elemental charge}^(-2))
	Lx = fX - iX
	Ly = fY - iY
	Lz = fZ - iZ
...
function write_angle_in_multiple_slices(atom_1, atom_2, iX, iY, iZ, fX, fY, fZ, Nz)

	results = process_gromacs_files()
	centre_of_mass_unk_dict = centre_of_mass_unk(results)
	electric_field_results, angle_results, magnitude_projection_results = electric_field_at_point_all(atom_1, atom_2, iX, iY, iZ, fX, fY, fZ)
    ...
    open("count_angle_in_multiple_slices.log", "w") do io
        ...
        for n in 1:Nz
            ...
            fX, fY, local_fZ)
				...
				p_magnitude = plot(title = "", xlabel = "Time (ps)", ylabel=L"Scalar Projection ($\mathrm{eV~e^{-1}~nm^{-1}}$)", legend = false, ylim=(-9, 9))
